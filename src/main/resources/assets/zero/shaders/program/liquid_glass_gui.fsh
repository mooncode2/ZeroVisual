...
vec3 sdgBox(in vec2 p, in vec2 b, vec4 ra) {
    ...
    // Inside the box, the analytic normal is max()-based, so it snaps between
    // horizontal and vertical along the corner diagonals — that crease shows up as
    // triangular / polygonal seams in the refraction on the panel sides. Replace it
    // with a creaseless soft-blend: weight the two edge normals by exp() proximity
    // to each edge, which is C-infinity smooth everywhere (no crease, no facets).
...
vec4 sampleBlur(int idx, vec2 uv) {
    // Never sample outside the framebuffer: off-screen texels are uninitialised
    // (black) and show up as a black rim/line where refraction pushes the sample
    // past a screen edge. Clamp keeps it on the blurred backdrop.
    uv = clamp(uv, 0.0, 1.0);
...
void main() {
    ...
    // Сначала вычисляем SDF стекла: он нужен и для раннего выхода, и для эффекта.
    // Пиксели вне всех виджетов (merged >= 0) — абсолютное большинство площади bbox,
    // поэтому раньше они зря прогоняли дорогой цикл теней и тригонометрию.
    ...
    // Тени рисуются на фоне под стеклом и вокруг него. Считаем их только если хоть
    // один виджет реально запросил тень (в Liquid Glass они выключены — тогда весь
    // цикл пропускается целиком).
    vec3 baseShadowed = base;
    if (merged >= 0.0) {
        bool anyShadow = false;
        for (int i = 0; i < MAX_WIDGETS; i++) {
            if (i >= n) break;
            if (Shadow0[i].y <= 0.0) continue;
        ...
        // Снаружи стекла: возвращаем фон (с тенью, если она была). Дорогая оптика ниже
        // не выполняется — это и есть основной выигрыш производительности.
    ...
    if (nmerged >= refThickness) {
        ...
        // Smoothly fade the refraction to zero as it approaches the inner edge of the
        // band (at refThickness = Edge Radius) instead of cutting it off hard. The hard
        // cutoff leaves a sharp rectangular seam where the band ends — that's the
        // unsmoothed "rects" artifact, and it grows more visible with larger Edge Radius.
    ...
    vec3 finalCol = mix(outColor.rgb, base, smoothstep(-0.001, 0.001, merged));
    // Per-widget opacity (Extra0.w): fade the whole glass effect toward the plain
    // backdrop so it disappears in sync with its HUD element instead of lingering.

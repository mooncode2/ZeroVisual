...
public final class Renderer2D {
   ...
   private final float[] scratchRadiiBuf = new float[4];
   private static final float[] CLIP_SCRATCH_RADII = new float[4];
   private Map<String, TextRenderer> idToTextRenderer = new HashMap<>();
   private final java.util.Map<String, TextRenderer.TextMetrics> textMeasureCache = new java.util.HashMap<>();
      ...
      if (width > 0 && height > 0) {
         ...
         try {
            ...
            if (this.backend != null) {
               ...
               ru.zero.util.color.ColorUtil.markFrame();
   ...
   public int[] getClipRectFramebuffer() {
      if (this.clipStack.isEmpty()) {
         return null;
      }
      Renderer2D.ClipState state = this.clipStack.peek();
      if (state == null || state.w() <= 0 || state.h() <= 0) {
         return null;
      }
      return new int[] {state.x(), state.y(), state.w(), state.h()};
   }
   ...
   public void text(FontObject fo, float x, float y, float size, String s, int rgbaPremul) {
      ...
      if (fo == null) {
         ...
         FontObject effectiveFont = ru.zero.util.render.text.FontRegistry.resolveForLanguage(fo);
         TextRenderer tr = this.idToTextRenderer.get(effectiveFont.id);
         if (tr != null) {
   ...
   public TextRenderer.TextMetrics measureText(FontObject fo, String text, float size) {
      if (fo == null) {
         ...
         if (tr == null) {
            ...
            String key = effectiveFont.id + '|' + size + '|' + content;
            TextRenderer.TextMetrics cached = this.textMeasureCache.get(key);
            if (cached != null) {
               return cached;
            }
            TextRenderer.TextMetrics result = tr.measureText(content, size / 2.0F);
            if (this.textMeasureCache.size() < 4096) {
               this.textMeasureCache.put(key, result);
            }
            return result;
         }
      }
   ...
   private static float[] scratchRadii(float[] buf, float topLeft, float topRight, float bottomRight, float bottomLeft) {
      buf[0] = topLeft;
      buf[1] = topRight;
      buf[2] = bottomRight;
      buf[3] = bottomLeft;
   ...
   private float[] scratchRadii(float topLeft, float topRight, float bottomRight, float bottomLeft) {
      return scratchRadii(this.scratchRadiiBuf, topLeft, topRight, bottomRight, bottomLeft);
   }
   ...
   private record ClipState(int x, int y, int w, int h, float roundTopLeft, float roundTopRight, float roundBottomRight,
      ...
      private static Renderer2D.ClipState fromRect(
         ...
         if (Float.isFinite(x) && Float.isFinite(y) && Float.isFinite(w) && Float.isFinite(h)) {
            ...
            float[] radii = Renderer2D.scratchRadii(Renderer2D.CLIP_SCRATCH_RADII, roundTopLeft, roundTopRight, roundBottomRight, roundBottomLeft);
   ...
   public static class ColorUtil {
      ...
      public Color interpolate(Color color1, Color color2, double amount) {
         ...
         return new Color(
               (int) Math.round(color1.getRed() + amount * (color2.getRed() - color1.getRed())),
               (int) Math.round(color1.getGreen() + amount * (color2.getGreen() - color1.getGreen())),
               (int) Math.round(color1.getBlue() + amount * (color2.getBlue() - color1.getBlue())),
               (int) Math.round(color1.getAlpha() + amount * (color2.getAlpha() - color1.getAlpha())));
      }
      ...
      public static int gradient2(int color1, int color2, int speed, int index) {
         ...
         float ratio = (float) ((angle % 360.0) / 360.0);
         ...
         int red = (int) (r1 * inv + r2 * ratio);
         int green = (int) (g1 * inv + g2 * ratio);
         int blue = (int) (b1 * inv + b2 * ratio);
         return 0xFF << 24 | (red & 0xFF) << 16 | (green & 0xFF) << 8 | (blue & 0xFF);
      }

      public static int interpolate(int color1, int color2, double amount) {
         amount = (float) MathHelper.clamp(amount, 0.0, 1.0);
         int r = (int) Math.round(red(color1) + amount * (red(color2) - red(color1)));
         int g = (int) Math.round(green(color1) + amount * (green(color2) - green(color1)));
         int b = (int) Math.round(blue(color1) + amount * (blue(color2) - blue(color1)));
         int a = (int) Math.round(alpha(color1) + amount * (alpha(color2) - alpha(color1)));
      ...
      public static int interpolateColor(int color1, int color2, double offset) {
         float r1 = red(color1) / 255.0F, g1 = green(color1) / 255.0F, b1 = blue(color1) / 255.0F, a1 = alpha(color1) / 255.0F;
         float r2 = red(color2) / 255.0F, g2 = green(color2) / 255.0F, b2 = blue(color2) / 255.0F, a2 = alpha(color2) / 255.0F;
         double r = r1 + (r2 - r1) * offset;
         double g = g1 + (g2 - g1) * offset;
         double b = b1 + (b2 - b1) * offset;
         double a = a1 + (a2 - a1) * offset;
      ...
      public static int skyRainbow(int speed, int index) {
         ...
         double var4;
         return Color.HSBtoRGB((var4 = angle % 360.0) / 360.0 < 0.5 ? -((float) (var4 / 360.0)) : (float) (var4 / 360.0),
               0.5F, 1.0F);
      }

...
public final class FogBlurPass {
   ...
   private final ShaderProgram program = ShaderProgram.fromResources(
      "assets/zero/shaders/blur/blur_fullscreen.vert", "assets/zero/shaders/postfx/fog_blur.frag"
   );
   private final ShaderProgram copyProgram = ShaderProgram.fromResources(
      "assets/zero/shaders/blur/blur_fullscreen.vert", "assets/zero/shaders/postfx/fog_copy.frag"
   );
   ...
   private final Matrix4f reusableProjectionMatrix = new Matrix4f();
   ...
   public FogBlurPass.Result render(
      ...
      if (colorBinding == null || colorBinding.textureId() <= 0 || width <= 0 || height <= 0) {
         ...
         Matrix4f projectionMatrix = sanitizedInverseProjectionValid ? this.reusableProjectionMatrix.set(inverseProjection) : this.reusableProjectionMatrix.identity();

...
public enum Easing {
   ...
   EASE_IN_OUT_BACK(
         x -> x < 0.5
               ? (2.0 * x) * (2.0 * x) * (7.189819 * x - 2.5949095) / 2.0
               : ((2.0 * x - 2.0) * (2.0 * x - 2.0) * (3.5949095 * (x * 2.0 - 2.0) + 2.5949095) + 2.0) / 2.0),
   ...
   SHRINK_EASING(x -> {
      ...
      double t = x - 1.0;
      return Math.max(0.0, 1.0 + shrink * t * t * t + easeAmount * t * t);
   });

   private final DoubleEasing function;

   private Easing(final DoubleEasing function) {
      this.function = function;
   }

   public DoubleEasing getFunction() {
      return this.function;
   }

   public double apply(double x) {
      return this.function.ease(x);
   }

   public float apply(float x) {
      return (float) this.function.ease(x);
   }

package ru.zero.util.render.math;
...
public class ScaledResolution {
   private static volatile Method cachedForceUnicodeFontMethod;
   private static volatile boolean methodLookupFailed;
   ...
   private static Method forceUnicodeFontMethod() {
      Method m = cachedForceUnicodeFontMethod;
      if (m != null || methodLookupFailed) {
         return m;
      }
      try {
         m = GameOptions.class.getMethod("getForceUnicodeFont");
      } catch (NoSuchMethodException var7) {
         try {
            m = GameOptions.class.getDeclaredMethod("forceUnicodeFont");
      ...
      cachedForceUnicodeFontMethod = m;
      return m;
   }
   ...
   public ScaledResolution(MinecraftClient mc) {
      if (mc != null && mc.getWindow() != null) {
         ...
         Method getMethod = forceUnicodeFontMethod();
         if (getMethod != null) {
            try {
               SimpleOption<Boolean> flag = (SimpleOption<Boolean>)getMethod.invoke(mc.options);

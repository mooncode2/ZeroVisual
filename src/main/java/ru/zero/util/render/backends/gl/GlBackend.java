...
public final class GlBackend implements RenderBackend {
   ...
   private final int[] flushPrevBindings = new int[16];
   ...
   public void endFrame() {
      this.flush();
      if (this.ssboSupported) {
         GL43.glBindBufferBase(37074, 0, 0);
      }
   ...
   public void flush() {
      if (this.batch.getInstanceCount() > 0) {
         ...
         int usedSlots = this.batch.getSlotCount();
         ...
         for (int slot = 0; slot < usedSlots; slot++) {
            ...
            int tex = this.batch.getSlotTexture(slot);
   ...
   public void prepareScreenBlur(int screenW, int screenH, float radiusPx) {
      if (screenW > 0 && screenH > 0) {
         RenderBackend.FrameCapture capture = this.captureFullFrame();
         if (capture.colorTexture() != 0 && capture.width() > 0 && capture.height() > 0) {
            // The blur pass temporarily binds its own framebuffer, shader and VAO.
            // Preserve the HUD renderer state so the queued static elements are
            // still drawn after this pass completes.
            int blurred = this.screenBlur.blurFromColorTexture(capture.colorTexture(), capture.width(), capture.height(),
                    Math.max(0.0F, radiusPx), true);
            if (blurred == 0) {
               ...
               this.preparedBlurW = capture.width();
               this.preparedBlurH = capture.height();
               this.preparedBlurScaleX = 1.0F;
               this.preparedBlurScaleY = 1.0F;
            }
         } else {
   ...
   public RenderBackend.FrameCapture captureFullFrame() {
      ...
      try {
         ...
         // The current draw framebuffer contains the already-rendered game scene.
         // Copying from it avoids sampling an unresolved/empty attachment texture.
         GL30.glBindFramebuffer(36008, state.drawFramebuffer);
         GL11.glReadBuffer(state.drawBuffer);
      ...
      return new RenderBackend.FrameCapture(this.fullFrameTarget.colorTex, this.fullFrameTarget.depthTex, width, height);
   }

   public void drawFullscreenTexture(int texture, int width, int height) {
      this.drawFullscreenTexture(texture, width, height, true);
   }

   public void drawFullscreenTextureToCurrentFb(int texture, int width, int height) {
      this.drawFullscreenTexture(texture, width, height, false);
   }

   private void drawFullscreenTexture(int texture, int width, int height, boolean rebindDefaultFb) {

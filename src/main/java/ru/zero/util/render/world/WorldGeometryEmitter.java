...
public final class WorldGeometryEmitter {
   ...
   public void emitLineWithoutNormal(Vec3d start, Vec3d end, int rgbaColor) {
      this.emitLineWithoutNormal(start, end, rgbaColor, rgbaColor);
   }

   public void emitLineWithoutNormal(Vec3d start, Vec3d end, int startColor, int endColor) {
      Objects.requireNonNull(start, "start");
      Objects.requireNonNull(end, "end");
      Vec3d cameraSpace = this.toCameraSpace(start);
      this.writeVertex(cameraSpace, startColor);
      cameraSpace = this.toCameraSpace(end);
      this.writeVertex(cameraSpace, endColor);
   }

   private void writeVertex(Vec3d cameraSpace, int rgbaColor) {
      VertexConsumer vertex = this.consumer.vertex(this.positionMatrix, (float)cameraSpace.x, (float)cameraSpace.y, (float)cameraSpace.z);

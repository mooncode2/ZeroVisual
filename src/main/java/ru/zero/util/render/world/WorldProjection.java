...
public final class WorldProjection {
   ...
   /** Screen position in framebuffer pixels for {@link ru.zero.util.render.core.Renderer2D}. */
   public static Vector2d projectFramebuffer(double x, double y, double z) {
      Vector2d scaled = project(x, y, z);
      if (scaled == null) {
         return null;
      }

      return new Vector2d(toFramebufferX((float) scaled.x), toFramebufferY((float) scaled.y));
   }

...
final class MsdfFont {
   private static final int DIRECT_GLYPH_RANGE = 256;
   private static final int ASCII_KERNING_RANGE = 128;
   private final Map<Integer, MsdfFont.Glyph> glyphs;
   private final MsdfFont.Glyph[] directGlyphs;
   private final Map<Long, Float> kerning;
   private MsdfFont fallbackFont;
   private final float[][] asciiKerning;
   ...
   private MsdfFont(
      ...
      this.glyphs = glyphs;
      this.directGlyphs = directGlyphs;
      this.kerning = kerning;
      this.asciiKerning = asciiKerning;
   }
   ...
   static MsdfFont load(RenderBackend backend, String jsonResourcePath, String textureResourcePath) {
      ...
      if (atlas == null) {
         ...
         if (metrics == null) {
            ...
            MsdfFont.Glyph[] directGlyphs = new MsdfFont.Glyph[DIRECT_GLYPH_RANGE];
            ...
            if (glyphArray != null) {
               for (JsonElement element : glyphArray) {
                  ...
                  if (codepoint >= 0 && codepoint < DIRECT_GLYPH_RANGE) {
                     directGlyphs[codepoint] = glyph;
                  }
               }
            }

            Map<Long, Float> kerning = new HashMap<>();
            float[][] asciiKerning = new float[ASCII_KERNING_RANGE][ASCII_KERNING_RANGE];
            ...
            if (kerningArray != null) {
               for (JsonElement element : kerningArray) {
                  ...
                  if (left >= 0 && left < ASCII_KERNING_RANGE && right >= 0 && right < ASCII_KERNING_RANGE) {
                     asciiKerning[left][right] = advance;
                  }
               }
            }
            ...
            return new MsdfFont(textureId, textureWidth, textureHeight, distanceRange, emSize, lineHeight, ascender, descender, glyphs, directGlyphs, kerning, asciiKerning);
         }
      }
   }
   ...
   MsdfFont.Glyph glyph(int codepoint) {
      if (codepoint >= 0 && codepoint < DIRECT_GLYPH_RANGE) {
         MsdfFont.Glyph g = this.directGlyphs[codepoint];
         if (g == null && this.fallbackFont != null) {
            return this.fallbackFont.glyph(codepoint);
         }
         return g;
      }
      MsdfFont.Glyph g = this.glyphs.get(codepoint);
      if (g == null && this.fallbackFont != null) {
         return this.fallbackFont.glyph(codepoint);
      }
      return g;
   ...
   void setFallback(MsdfFont fallback) {
      this.fallbackFont = fallback;
   }

   int textureForCodepoint(int codepoint) {
      boolean inMain;
      if (codepoint >= 0 && codepoint < DIRECT_GLYPH_RANGE) {
         inMain = this.directGlyphs[codepoint] != null;
      } else {
         inMain = this.glyphs.get(codepoint) != null;
      }
      if (!inMain && this.fallbackFont != null) {
         return this.fallbackFont.textureId();
      }
      return this.textureId;
   }
   ...
   float kerning(int left, int right) {
      if (left >= 0 && left < ASCII_KERNING_RANGE && right >= 0 && right < ASCII_KERNING_RANGE) {
         return this.asciiKerning[left][right];
      }

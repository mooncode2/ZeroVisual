      ...
      if (!(size <= 0.0F)) {
         ...
         if (!content.isEmpty()) {
            ...
            if (content.indexOf(10) < 0) {
               float width = this.measureLineWidth(content, scale);
               float startX = x;
               if ("c".equals(align)) {
                  startX = x - width * 0.5F;
               ...
               this.drawTextLine(startX, baselineY, scale, content, color, matrix, texture, pxRange);
            } else {
      ...
      if (!line.isEmpty()) {
         ...
         while (i < line.length()) {
            ...
            if (ch == '\\' && i + 9 < line.length() && line.charAt(i + 1) == 'c') {
               ...
               int glyphCode = cp;
               int glyphTexture = this.font.textureForCodepoint(cp);
               if (glyph == null) {
               ...
               if (glyph.renderable) {
                  ...
                  if (width > 0.0F && height > 0.0F) {
                     this.backend.enqueueMsdfGlyph(glyphTexture, pxRange, x0, y0, width, height, glyph.u0, glyph.v1, glyph.u1, glyph.v0, color, matrix);
                  }
               }
   ...
   public TextRenderer.TextMetrics measureText(String text, float size) {
      if (size <= 0.0F) {
         ...
         if (content.isEmpty()) {
            ...
            int lineCount;
            if (content.indexOf(10) < 0) {
               maxWidth = this.measureLineWidth(content, scale);
               lineCount = 1;
            } else {
               String[] lines = content.split("\n", -1);

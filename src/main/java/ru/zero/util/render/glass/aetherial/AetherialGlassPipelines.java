...
public final class AetherialGlassPipelines {
   private static RenderPipeline guiPipeline;

   private AetherialGlassPipelines() {
   }

   public static synchronized RenderPipeline getGuiPipeline() {
      if (guiPipeline == null) {
         guiPipeline = RenderPipeline.builder(new RenderPipeline.Snippet[0]).withLocation(Identifier.of("aetherial", "pipeline/liquid_glass_gui")).withVertexShader(Identifier.of("aetherial", "core/blit_quad")).withFragmentShader(Identifier.of("aetherial", "program/liquid_glass_gui")).withUniform("Projection", UniformType.UNIFORM_BUFFER).withUniform("SamplerInfo", UniformType.UNIFORM_BUFFER).withUniform("CustomUniforms", UniformType.UNIFORM_BUFFER).withUniform("WidgetInfo", UniformType.UNIFORM_BUFFER).withUniform("BgConfig", UniformType.UNIFORM_BUFFER).withSampler("Sampler0").withSampler("Sampler1").withSampler("Sampler2").withSampler("Sampler3").withSampler("Sampler4").withSampler("Sampler5").withDepthTestFunction(DepthTestFunction.NO_DEPTH_TEST).withDepthWrite(false).withVertexFormat(VertexFormats.POSITION, VertexFormat.DrawMode.QUADS).build();

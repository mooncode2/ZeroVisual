...
public final class ColorUtil {
   ...
   public static int interpolateInt(int oldValue, int newValue, double interpolationValue) {
      return (int) interpolateD(oldValue, newValue, (float)interpolationValue);
   }
   ...
   public static int overCol(int color1, int color2, float percent01) {
      double percent = MathHelper.clamp(percent01, 0.0F, 1.0F);
      int r = (int) Math.round(red(color1) + percent * (red(color2) - red(color1)));
      int g = (int) Math.round(green(color1) + percent * (green(color2) - green(color1)));
      int b = (int) Math.round(blue(color1) + percent * (blue(color2) - blue(color1)));
      int a = (int) Math.round(alpha(color1) + percent * (alpha(color2) - alpha(color1)));
      return getColor(r, g, b, a);
   }
   ...
   private static volatile int fadeGeneration = 0;
   private static int cachedFadeGeneration = -1;
   private static long cachedFadeTimeNanos = 0L;
   private static int cachedFadeValue = 0;
   private static final long FADE_CACHE_TTL_NS = 16_000_000L;
   ...
   public static int fade() {
      int gen = fadeGeneration;
      long now = System.nanoTime();
      if (gen == cachedFadeGeneration && now - cachedFadeTimeNanos < FADE_CACHE_TTL_NS) {
         return cachedFadeValue;
      }
      int value = Renderer2D.ColorUtil.getClientColor();
      cachedFadeValue = value;
      cachedFadeGeneration = gen;
      cachedFadeTimeNanos = now;
      return value;
   }
   ...
   public static int gradient(int start, int end, int index, int speed) {
      ...
      float[] hsb = Color.RGBtoHSB((int)(r * 255.0F), (int)(g * 255.0F), (int)(b * 255.0F), HSB_SCRATCH);
      hsb[1] *= 1.5F;

...
@IModule(
   name = "AutoResell",
   description = "Автоматически перевыставляет предметы на аукционе FunTime",
   category = Category.FunTime,
   bind = -1
)
@Environment(EnvType.CLIENT)
public class AutoResell extends Module {
   /** Номера слотов в меню аукциона (с 1, как в игре). */
   ...
   private void tickWaitInterval() {
      ...
      if (!warnedTenSeconds && waitTicks >= warningAt && intervalTicks > 200) {
         ...
         FunTimeUtil.notifyPlayer("Через 10 секунд предметы будут перевыставлены");
      }

      if (waitTicks >= intervalTicks) {
         if (!inPvpMode.get() && FunTimeUtil.hasRedBossBar()) {
            FunTimeUtil.notifyFailure("Перевыставление неуспешно: активен PvP (красный BossBar)");
            resetCycle();
            return;
         }
         startResellSequence();
   ...
   private void tickOpenAh() {
      ...
      if (!(mc.currentScreen instanceof HandledScreen<?>)) {
         if (actionTicks >= OPEN_TIMEOUT_TICKS) {
            failResell("не удалось открыть аукцион (/ah)");
         }
         return;
      }
   ...
   private void tickActionDelay(Phase next) {
      if (!(mc.currentScreen instanceof HandledScreen<?>)) {
         failResell("аукцион закрылся до завершения");
         return;
      }
   ...
   private void tickClose() {
      ...
      if (clickedEnderChest && clickedClock) {
         FunTimeUtil.notifySuccess("Перевыставление успешно");
         resetCycle();
      } else {
         failResell("не все действия в аукционе выполнены");
      }
   }

   private void failResell(String reason) {
      FunTimeUtil.notifyFailure("Перевыставление неуспешно: " + reason);
      if (mc.currentScreen != null) {
         mc.player.closeHandledScreen();
      }
      resetCycle();

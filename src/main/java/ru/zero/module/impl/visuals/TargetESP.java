...
import ru.zero.util.render.world.CrystalRenderer;
import org.joml.Quaternionf;
import org.joml.Vector3f;

/**
 * Подсветка цели только по {@link net.minecraft.client.MinecraftClient#targetedEntity} (Vanilla), без боевых модулей.
 */
@IModule(name = "TargetESP", description = "Не таргет есп", category = Category.Visuals, bind = -1)
...
public class TargetESP extends Module {
   ...
   public static SliderSetting circleSize = new SliderSetting("Размер круга", 1.2F, 0.5F, 3.0F, 0.1F, false)
         .hidden(() -> !typeTargetEsp.is("Круг"));
   private static final Identifier TARGET_TEXTURE = Identifier.of("zero", "textures/world/target.png");
   private static final Identifier TARGET_TEXTURE_C = Identifier.of("zero", "textures/world/targetn2.png");
   private static final Identifier TARGET_TEXTURE_N = Identifier.of("zero", "textures/world/targetn.png");
   private static final Identifier GLOW_TEXTURE = Identifier.of("zero", "textures/world/glow.png");
   ...
   private float crystalOrbit;
   private static final Vector3f CRYSTAL_UP = new Vector3f(0.0F, 1.0F, 0.0F);
   ...
   private static final RenderPipeline RING_LINE_PIPELINE = RenderPipelines.register(
         RenderPipeline.builder(new Snippet[] { RenderPipelines.POSITION_COLOR_SNIPPET })
               ...
               .withBlend(BlendFunction.LIGHTNING)
               .build());
   private static final RenderLayer RING_STRIP_LAYER = RenderLayer.of("zero_ring_strip", RenderSetup.builder(RING_STRIP_PIPELINE).expectedBufferSize(1024).translucent().build());
   private static final RenderLayer RING_LINE_LAYER = RenderLayer.of("zero_ring_line", RenderSetup.builder(RING_LINE_PIPELINE).expectedBufferSize(1024).translucent().build());
   private static final RenderPipeline COLOR_QUADS_PIPELINE = RenderPipelines.register(
         RenderPipeline.builder(new Snippet[] { RenderPipelines.POSITION_COLOR_SNIPPET })
               .withLocation(Identifier.of("zero", "pipeline/world/color_quads"))
               ...
               .withBlend(BlendFunction.LIGHTNING)
               .build());
   private static final RenderLayer COLOR_QUADS_LAYER = RenderLayer.of("zero_color_quads", RenderSetup.builder(COLOR_QUADS_PIPELINE).expectedBufferSize(1024).translucent().build());
   ...
   private static final RenderPipeline CUBE_LINES_PIPELINE = RenderPipelines.register(
         RenderPipeline.builder(new Snippet[] { RenderPipelines.POSITION_COLOR_SNIPPET })
               .withLocation(Identifier.of("zero", "targetesp_cube_lines"))
   ...
   private static final RenderPipeline CRYSTAL_TRIANGLES_PIPELINE = RenderPipelines.register(
         RenderPipeline.builder(new Snippet[] { RenderPipelines.POSITION_COLOR_SNIPPET })
               .withLocation(Identifier.of("zero", "targetesp_crystal_triangles"))
   ...
   private static final RenderLayer CRYSTAL_LAYER = RenderLayer.of(
         "zero/target_esp/crystals",
         RenderSetup.builder(CRYSTAL_TRIANGLES_PIPELINE).expectedBufferSize(4096).translucent().build());

   public TargetESP() {
      this.addSettings(new Setting[] { typeTargetEsp, typeGhost, typeImage, typeCube, circleSize });
   }
         ...
         if (alpha.getValue() > 0.0) {
            ...
            if (this.lastTarget != null
               ...
               if (typeTargetEsp.is("Картинка") && typeImage.is("Ромб")) {
                  this.renderDiamond(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Картинка") && typeImage.is("Клиент")) {
                  this.renderDiamondNewStyle(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Картинка") && typeImage.is("Ромб 2")) {
                  this.renderDiamondNewStyle2(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Призраки") && typeGhost.is("Обычный")) {
                  this.renderGhosts(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Призраки") && typeGhost.is("Новый")) {
                  this.renderSpirits(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Призраки") && typeGhost.is("Старый")) {
                  this.renderSpiritsOld(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Кольцо")) {
                  this.renderRing(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Кубики") && typeCube.is("Новый")) {
                  this.renderCubes(stack, immediate, this.lastTarget, tickDelta);
               }

               if (typeTargetEsp.is("Кубики") && typeCube.is("Старый")) {
                  this.renderCubesOld(stack, immediate, this.lastTarget, tickDelta);
               }


               if (typeTargetEsp.is("Круг")) {
                  this.render3DCircle(stack, immediate, this.lastTarget, tickDelta);
               }
            }
         } else {
            this.lastTarget = null;
   ...
   private void render3DCircle(MatrixStack matrices, Immediate immediate, LivingEntity target, float partialTicks) {
      Vec3d lerped = target.getLerpedPos(partialTicks);
      Vec3d cameraPos = mc.gameRenderer.getCamera().getCameraPos();
      float animation = (float) alpha.getValue();
      if (animation <= 0.0F) {
         return;
      }

      // Calculate time-based animation for up/down movement
      long currentTime = System.currentTimeMillis();
      float time = (currentTime % 2000L) / 2000.0F;
      float yOffset = (float) Math.sin(time * Math.PI * 2) * 0.5F;

      matrices.push();
      matrices.translate(lerped.x - cameraPos.x, lerped.y - cameraPos.y + target.getHeight() / 2.0F + yOffset, lerped.z - cameraPos.z);

      // Draw the 3D circle (now fixed to entity, not rotating with camera)
      int segments = 64;
      float radius = target.getWidth() * circleSize.get();
      int color = ColorUtil.replAlpha(ColorUtil.fade(), (int) (255.0F * animation));
      int r = color >> 16 & 0xFF;
      int g = color >> 8 & 0xFF;
      int b = color & 0xFF;
      int a = color >> 24 & 0xFF;
      ...
      VertexConsumer buffer = immediate.getBuffer(RING_STRIP_LAYER);

      for (int i = 0; i <= segments; i++) {
         float angle = (float) (i * 2 * Math.PI / segments);
         float x = (float) (radius * Math.cos(angle));
         float z = (float) (radius * Math.sin(angle));
         float y = (float) (Math.sin(angle * 4) * 0.1F); // Small wave effect

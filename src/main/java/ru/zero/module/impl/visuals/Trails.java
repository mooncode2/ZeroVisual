...
import ru.zero.util.render.math.animation.AnimationUtils;
import ru.zero.util.render.math.animation.anim2.Animation;
import ru.zero.util.render.math.animation.anim2.Easing;
import ru.zero.util.render.math.animation.anim2.Interpolator;
...
public class Trails extends Module {
   ...
   public void onEvent(EventMotion event) {
      ...
      if (player != null) {
         ...
         float delayVal = delay.get();
         this.particles.removeIf(particle -> particle.getTimerUtil().hasReached((long) delayVal)
               || particle.position.distance(currentX, currentY, currentZ) >= 100.0);
      }
   }
   ...
   public void onEvent(WorldRenderEvent event) {
      if (!this.particles.isEmpty()) {
         ...
         if (mc.options.getPerspective() != Perspective.FIRST_PERSON || firstPerson.get()) {
            ...
            int particleCount = this.particles.size();

            for (int i = 0; i < particleCount; i++) {
               Trails.Particle3D particle = this.particles.get(i);
               ...
               if ((int) particle.getAnimation().getValue() != 0
                     && particle.getTimerUtil().hasReached((long) (delayVal - 400.0F))) {
               ...
               int color3 = color[2];
               int color4 = color[3];
               Trails.Particle3D prevParticle = i > 0 ? this.particles.get(i - 1) : null;

               double bloomDist = prevParticle != null ? particle.position.distance(prevParticle.position) : 0.0;
               ...
               float bloomSize = (float) clamp(0.5, 4.0, bloomDist * 6.0);
   ...
   private static class Particle3D {
      ...
      public static Block getBlock(double x, double y, double z) {
         MUTABLE_POS.set((int) x, (int) y, (int) z);
         return mc.world.getBlockState(MUTABLE_POS).getBlock();
      }

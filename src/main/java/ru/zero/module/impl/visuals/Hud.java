...
import ru.zero.module.impl.visuals.HUD.KeyBindHUD;
import ru.zero.module.impl.visuals.HUD.BossBarHUD;
import ru.zero.module.impl.visuals.HUD.PotionsHUD;
import ru.zero.module.impl.visuals.HUD.ScoreboardHUD;
import ru.zero.module.impl.visuals.HUD.TargetHUD;
import ru.zero.module.impl.visuals.HUD.WaterMark;
...
import ru.zero.util.render.core.Renderer2DAdapter;
...
public class Hud extends Module {
   ...
   public Hud() {
      ...
      DraggableManager.getInstance().setYPinned("notifications");
   }
       ...
       if (mc.player != null && mc.world != null && !mc.options.hudHidden) {
         ...
         if (r2 != null) {
            ...
            if (element.get("Ватер марк")) {
               HudEditor.renderElement("watermark", r2, () -> WaterMark.waterMark(r2));
            }
            ...
            if (element.get("Список зелий") && HudOverlayHelper.shouldUseCustomPotions()) {
               HudEditor.renderElement("potions", r2, () -> PotionsHUD.potions(r2, e.drawContext()));
            }

            if (element.get("Хот бар") && HudOverlayHelper.shouldUseCustomHotbar()) {
               HudEditor.renderElement("hotbar", r2, () -> HotBarHUD.hotbar(r2, e.drawContext()));
            }

            if (element.get("Скорборд") && HudOverlayHelper.shouldUseCustomScoreboard()) {
               HudEditor.renderElement("scoreboard", r2, () -> ScoreboardHUD.scoreboard(r2));
            }

            if (element.get("Босс бар") && HudOverlayHelper.shouldUseCustomBossBar()) {
               HudEditor.renderElement("bossbar", r2, () -> BossBarHUD.bossbar(r2));
            }
   ...
   public void renderNotifications(Renderer2D matrix) {
      ...
      if (!this.notifications.isEmpty()) {
         ...
         for (int i = this.notifications.size() - 1; i >= 0; i--) {
            ...
            if (!(animValue <= 0.01F)) {
               ...
               notification.measuredWidth = textWidth;
         ...
         float currentY = dragSession.positionY();
          ...
          for (int i = this.notifications.size() - 1; i >= 0 && rendered < maxNotifications; i--) {
             ...
             if (!(animValue <= 0.01F)) {
                float iconWidth = 8.0F;
                float textWidth = notification.measuredWidth;
                float notificationWidth = margin * 3.0F + iconWidth + textWidth + 8.0F + 30.0F;
     ...
     public static void drawClientRect(Renderer2D r2, float x, float y, float w, float h, float radius, float alpha, float thickness) {
        if (ru.zero.util.render.glass.LiquidGlassRenderer.isEnabled()) {
          ...
          boolean blurEnabled = blur.get() && !ru.zero.compat.LunarCompat.isLunarClient();
          if (blurEnabled) {
             float blurRadius = Optimizer.optimizeHudBlur(23.0F);
             if (alpha < 0.12F) {
                blurRadius = Math.min(blurRadius, 8.0F);
             }

             r2.prepareBlur(blurRadius);
          ...
          r2.rectOutline(
             ...
             h + 2.0F,
             radius,
             ColorUtil.replAlpha(Renderer2D.ColorUtil.getOutLineColor(1, 1), alpha * 0.1F),
             thickness
          );
          // Без блюра фон должен быть достаточно плотным, иначе HUD «пропадает» на
          // ярком фоне игры. С блюром — полупрозрачный, чтобы размытие просвечивало.
          float bgOpacity = blurEnabled ? 0.7F : 0.92F;
          r2.rect(x, y, w, h, radius, ColorUtil.replAlpha(Renderer2D.ColorUtil.getBackGroundColor(1, 1), alpha * bgOpacity));
       }
    }

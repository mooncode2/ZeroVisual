            ...
            if (effectx != null) {
               ...
               if (alphaAnim != null) {
                  ...
                  if (currentAlpha <= 0.01F) {
                     ...
                     // Center the 10x10 icon inside the left block (before the vertical divider).
                     float iconX = x + 14.0F + x3;
                     float iconY = currentAnimatedY + 10.0F;
                     float guiScale = mc.getWindow() != null ? mc.getWindow().getScaleFactor() : 1.0F;
                     float originX = x / guiScale;
                     float originY = y / guiScale;
                     float drawIconX = iconX / guiScale;
                     float drawIconY = iconY / guiScale;
                     drawContext.getMatrices().pushMatrix();
                     drawContext.getMatrices().translate(originX, originY);
                     drawContext.getMatrices().scale(hudScale, hudScale);
                     drawContext.getMatrices().translate(-originX, -originY);
                     drawContext.drawGuiTexture(RenderPipelines.GUI_TEXTURED, effectTexture, (int) drawIconX, (int) drawIconY, 10, 10);

...
public class HotBarHUD {
   ...
   public static void hotbar(Renderer2D r2, DrawContext drawContext) {
      if (mc.player != null && mc.world != null) {
         ...
         float defaultY = screenHeight - hotbarHeight - 10.0F;
         DraggableManager.DragSession dragSession = DraggableManager.getInstance().beginDrag("hotbar", defaultX, defaultY, hotbarWidth, hotbarHeight);
         float x = dragSession.positionX();
         float y = dragSession.positionY();
         float deltaFbX = x - defaultX;
         float deltaFbY = y - defaultY;
         ...
         if (deltaScX != 0.0F || deltaScY != 0.0F || hasStatusBars) {
            ...
            if (deltaScX != 0.0F || deltaScY != 0.0F) {
               drawContext.getMatrices().translate(deltaScX, deltaScY);
            }

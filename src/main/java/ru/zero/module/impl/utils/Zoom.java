...
public class Zoom extends Module {
   ...
   public float modifyFov(float vanillaFov, float tickDelta, boolean changingFov) {
      if (!this.enable || mc == null || mc.getWindow() == null) {
       ...
       boolean hold = !inScreen && InputUtil.isKeyPressed(mc.getWindow(), key);
      float dt = Math.max(0.0F, tickDelta);
      double speed = zoomSpeed.get();
      float step = 1.0F - (float) Math.exp(-dt * speed * 0.35F);
      step = MathHelper.clamp(step, 0.0F, 1.0F);

      if (hold) {
         this.blend = MathHelper.lerp(step, this.blend, 1.0F);
      } else {
         float releaseStep = Math.min(1.0F, step * 2.25F);
         this.blend = MathHelper.lerp(releaseStep, this.blend, 0.0F);
         if (this.blend < 0.06F) {
            this.blend = 0.0F;
         }
      }

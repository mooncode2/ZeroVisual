...
import ru.zero.util.client.ModuleSounds;
...
public class Module extends Config {
   ...
   private ru.zero.util.render.math.animation.Animation animation1;
   private ru.zero.util.render.math.animation.Animation animation2;
   public final Animation2 mAnim = new Animation2();
   private ru.zero.util.render.math.animation.Animation animation3;
   private ru.zero.util.render.math.animation.Animation animation4;
   private static boolean configLoadInProgress;

   public ru.zero.util.render.math.animation.Animation getAnimation1() {
      if (animation1 == null) animation1 = new EaseInOutQuad(300, 1.0);
      return animation1;
   }

   public ru.zero.util.render.math.animation.Animation getAnimation2() {
      if (animation2 == null) animation2 = new EaseInOutQuad(300, 1.0);
      return animation2;
   }

   public ru.zero.util.render.math.animation.Animation getAnimation3() {
      if (animation3 == null) animation3 = new EaseInOutQuad(300, 1.0);
      return animation3;
   }

   public ru.zero.util.render.math.animation.Animation getAnimation4() {
      if (animation4 == null) animation4 = new EaseInOutQuad(300, 1.0);
      return animation4;
   }
   ...
   public void onEnable() {
      ...
      if (mc.player != null && !configLoadInProgress) {
         Zero.get.manager.get(Hud.class).showNotification(this.name + Lang.t(" включен"), Hud.NotificationType.SUCCESS, 2400L);
         ModuleSounds.playToggle(true);
   ...
   public void onDisable() {
      ...
      if (mc.player != null && !configLoadInProgress) {
         Zero.get.manager.get(Hud.class).showNotification(this.name + Lang.t(" выключен"), Hud.NotificationType.WARNING, 2400L);
         ModuleSounds.playToggle(false);
   ...
   public void toggle() {
      ...
      if (!configLoadInProgress && Zero.get.configManager != null) {
   ...
   public JsonObject save() {
      ...
      for (Setting set : this.getSettings()) {
         if (set instanceof BooleanSetting) {
            ...
            if (hueSetting.alphaEnabled) {
               colorObject.addProperty("alpha", hueSetting.alpha);
            }
            ...
            if (set != null && propertiesObject != null && propertiesObject.has(set.name)) {
               if (set instanceof BooleanSetting) {
                  ...
                  if (propertiesObject.get(set.name).isJsonObject()) {
                     ...
                     if (colorObject.has("alpha")) {
                        hueSetting.alpha = colorObject.get("alpha").getAsFloat();
                     }
                  } else {

...
public final class FriendCommand implements Command {
   ...
   private static final List<String> COMMAND_ALIASES = List.of(".fried");
   ...
   @Override
   public String usage() {
      return ".fried <add/remove/list> [ник]";
   }
   ...
   public void execute(CommandContext context, String arguments) throws CommandException {
      if (Zero.get == null || Zero.get.friendManager == null) {
         throw new CommandException("Friend system is not ready yet");
      }
      if (arguments == null || arguments.isBlank()) {
         context.sendInfo("Usage: .fried add <ник>, .fried remove <ник>, .fried list");
         return;
      }
   ...
   private void handleAdd(CommandContext context, String name) throws CommandException {
      if (name == null || name.isBlank()) {
         throw new CommandException("Specify player name: .fried add <ник>");
      }
      FriendManager manager = Zero.get.friendManager;
      if (manager.isFriend(name)) {
         context.sendInfo("'" + name + "' is already in friends");
         return;
      }
      manager.add(name);
   ...
   private void handleRemove(CommandContext context, String name) throws CommandException {
      if (name == null || name.isBlank()) {
         throw new CommandException("Specify player name: .fried remove <ник>");
      }
      FriendManager manager = Zero.get.friendManager;
      if (!manager.isFriend(name)) {
         throw new CommandException("'" + name + "' is not in friends");
      }
      manager.remove(name);

...
public final class TargetCommand implements Command {
   ...
   private static final String USAGE = ".target add " + Lang.t("<ник>") + " | .target remove " + Lang.t("<ник>") + " | .target list";
   ...
   public String description() {
      return Lang.t("Управление списком целей");
   }
   ...
   public void execute(CommandContext context, String arguments) throws CommandException {
      if (Zero.get == null || Zero.get.targetManager == null) {
         throw new CommandException(Lang.t("Система целей ещё не загружена"));
      }
   ...
   private void handleAdd(CommandContext context, String name) throws CommandException {
      if (name == null || name.isBlank()) {
         throw new CommandException("Укажи ник: .target add " + Lang.t("<ник>"));
      }

      TargetManager manager = Zero.get.targetManager;
      if (manager.isTarget(name)) {
         context.sendInfo("'" + name + "'" + Lang.t(" уже в списке целей"));
         return;
      }

      manager.add(name);
      context.sendSuccess(Lang.t("Добавлена цель: ") + name);
   }

   private void handleRemove(CommandContext context, String name) throws CommandException {
      if (name == null || name.isBlank()) {
         throw new CommandException("Укажи ник: .target remove " + Lang.t("<ник>"));
      }

      TargetManager manager = Zero.get.targetManager;
      if (!manager.isTarget(name)) {
         throw new CommandException("'" + name + "'" + Lang.t(" нет в списке целей"));
      }

      manager.remove(name);
      context.sendSuccess(Lang.t("Удалена из целей: ") + name);
   }

   private void handleList(CommandContext context) {
      List<Target> targetList = TargetManager.getTargets();
      if (targetList.isEmpty()) {
         context.sendInfo(Lang.t("Список целей пуст"));
         return;
      }

      MutableText builder = Text.literal(Lang.t("Цели: "));

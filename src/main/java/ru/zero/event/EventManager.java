...
public class EventManager {
   ...
   public static void register(Object object) {
      for (Method method : object.getClass().getDeclaredMethods()) {
         if (method.isAnnotationPresent(EventInit.class) 
   ...
   private static void register(Method method, Object object) {
      try {
         ...
         final EventManager.MethodData data = new EventManager.MethodData(object, method, handle, method.getAnnotation(EventInit.class).value());
         ...
         System.err.println("Failed to unreflect event handler: " + method.getName() + " in " + object.getClass().getName());
   ...
   private static void invoke(EventManager.MethodData data, Event argument) {
      try {
         data.getHandle().invoke(data.getSource(), argument);
      } catch (Throwable var4) {
         Throwable cause = var4 instanceof java.lang.reflect.InvocationTargetException ? var4.getCause() : var4;
         System.err
            .println(
               "[EventManager] Exception in handler "
                  ...
                  + ": "
                  + (cause != null ? cause.getMessage() : var4.getMessage())
            );
         if (cause != null) {
   ...
   private static final class MethodData {
      ...
      public MethodData(Object source, Method target, MethodHandle handle, byte priority) {
         this.source = source;
         this.target = target;
         this.handle = handle;

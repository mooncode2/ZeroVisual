...
public final class GuiServerMapPanel extends GuiScreen {
   ...
   public static void renderPanel(Renderer2D renderer2D, MatrixStack pose, float mainAlpha) {
      ...
      renderer2D.text(FontRegistry.INTER_MEDIUM, panelX + 10.0F, panelY + 8.0F + 7.0F, 14.0F, "Карта сервера", textColor);
      if (TILE_VIEW.hasTiles()) {
         renderTiles(renderer2D, mapX, mapY, mapW, mapH, mainAlpha);
      } else if (TILE_VIEW.isLoading()) {
         renderer2D.text(FontRegistry.INTER_MEDIUM, mapX + 12.0F, mapY + mapH / 2.0F, 14.0F, "Загрузка карты...", textMuted);
      } else {
         renderer2D.text(FontRegistry.INTER_MEDIUM, mapX + 12.0F, mapY + mapH / 2.0F - 10.0F, 13.0F, "Превью недоступно", textMuted);
         renderer2D.text(FontRegistry.INTER_MEDIUM, mapX + 12.0F, mapY + mapH / 2.0F + 6.0F, 12.0F, "Используйте кнопку ниже", textMuted);
      }
      ...
      float textW = renderer2D.measureText(FontRegistry.INTER_MEDIUM, "Открыть в браузере", 13.0F).width;
      renderer2D.text(
